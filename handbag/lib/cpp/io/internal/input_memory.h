...
public:
explicit InMemoryInputStream(T data) noexcept(std::is_nothrow_move_constructible_v<T>)
    : data_(std::move(data)) {}
...
absl::StatusOr<size_t> read(void* const dst, const size_t dst_capacity) noexcept override {
    ...
    const auto bytes_to_read = bytes_available < dst_capacity ? bytes_available : dst_capacity;
