...
constexpr bool should_return_by_value() noexcept {
  ...
  const bool is_memory =
      sizeof(T) >= 4 * sizeof(size_t) && !std::is_trivially_copyable_v<T>;
...
#define _HANDBAG_FLUENT_MEMBER_IMPL_1(type_) \
  static_assert(false,                       \
                "`HANDBAG_FLUENT_MEMBER` can only have 2 or 3 arguments");
...
#define _HANDBAG_FLUENT_MEMBER_IMPL_2(type_, name)                             \
 private:                                                                      \
  ...
  static_assert(                                                               \
      ...
      "Please use HANDBAG_FLUENT_MEMBER with 3 arguements; "                   \
      "`" HANDBAG_STRINGIZE(type_) "` is trivial and must be initialized");    \
                                                                               \
 public:                                                                       \
  auto& set_##name(::handbag::fluent::internal::FluentParam<type_>             \
                       new_value)& noexcept(::handbag::fluent::internal::      \
                                                is_noexcept_setter<type_>()) { \
    _handbag_fluent_member_##name##_ = std::move(new_value);                   \
    return *this;                                                              \
  }                                                                            \
                                                                               \
  auto&& set_##name(                                                           \
      ::handbag::fluent::internal::FluentParam<type_>                          \
          new_value)&& noexcept(::handbag::fluent::internal::                  \
                                    is_noexcept_setter<type_>()) {             \
    _handbag_fluent_member_##name##_ = std::move(new_value);                   \
  ...
  const auto* ptr_##name() const noexcept {                                    \
    return &_handbag_fluent_member_##name##_;                                  \
                                                                               ...
                                                                               \
  auto* mut_##name() noexcept { return &_handbag_fluent_member_##name##_; }    \
                                                                               \
